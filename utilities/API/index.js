...
export const resendReport = async () => {
  ...
  DeviceEventEmitter.addListener("AllMediaResolved", (data) => {
    ...
    if(typeof savedReport === "string" && savedReport !== "") {
      ...
      fetch("http://192.168.43.196:3000/api/report/new", {
        method: "POST",
...
export const uploadMediaFile = async (mediaType, mediaUri, updateMedia) => {
  ...
  xhr.open("POST", "http://192.168.43.196:3000/cdn/upload/" + mediaType);
...
export const fileReport = async (report, onSuccess, onErr) => {
  ...
  // do a garbage collection of eventListeners
    ...
    DeviceEventEmitter.addListener("AllMediaResolved", (data) => {
      ...
      fetch(
        "http://192.168.43.196:3000/api/report/new",
        {
          method: "POST",
          body: JSON.stringify(sendableReport),
          headers: {
...
export const sendCulpritInformation = (report_id, reportData, onSuccess, onErr) => {
  ...
  fetch(
    "http://192.168.43.196:3000/api/report/" + report_id + "/add/culpritInformation",
    {
      method: "PUT",
...
export const updateMatatuDetails = (report_id, payload, onSuccess, onErr) => {
  ...
  fetch(
    "http://192.168.43.196:3000/api/report/" + report_id + "/add/matatuDetails",
    {
      method: "PUT",
...
export const isUser = (email, onSuccess, onErr) => {
  ...
  fetch(`http://192.168.43.196:3000/api/user/isUser/${email}`)
...
export const getLoginDetails = (email, onSuccess, onErr) => {
  ...
  fetch(`http://192.168.43.196:3000/api/user/login?email=${email}`)
    .then(response => response.json())
    .then(data => {
...
export const fetchRouteReports = (route_id, onSuccess, onErr) => {
  ...
  fetch(`http://192.168.43.196:3000/api/routes/${route_id}/reports`)
    .then(response => response.json())
    .then(data => {
...
/**
 * http://192.168.43.196/find/categories=route_id;date;location_type;flags&route_id={string}&date={timestamp}:{timestamp}&location_type={string}&flags=Verbal;Non-verbal;Physical&key={string}
 * 
...
export const filterByCategories = (filterObject, onSuccess, onErr) => {
  ...
  fetch(`http://192.168.43.196:3000/api/report/find${generateCategories()}${generateCategoryValues()}`)
    .then(response => response.json())
    .then(data => {
...
export const fetchReportsByRoute = (route_id, onSuccess, onErr) => {
  ...
  fetch(
    "http://192.168.43.196:3000/api/report/find?categories=route_id&route_id=" + route_id
  ).then(response => response.json()).then(data => {
...
export const queryReports = (categories, valuesObj, onSuccess, onErr) => {
  ...
  fetch(
    `http://192.168.43.196:3000/api/reports/find?${unpackCategories()}${unpackCategoryValues()}`
  ).then(response => response.json())
    .then(data => {
